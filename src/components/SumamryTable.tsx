...
import { generateRangeFromYear } from "../utils/generate-dates-from-year"
import { HabitDay } from "./HabitDay"

const weekDays = ['D','S','T','Q','Q','S','S']

const summaryDates = generateRangeFromYear()
        ...
        <div className="w-full flex">
            <div className="grid grid-rows-7 grid-flow-row gap-3">
                {weekDays.map((weekDay,i)=>{
                    return(
                        <div key={`${weekDay}-${i}`}
                        className="text-zinc-400 text-xl h-10 w-10 font-blod flex item-center justify-center">
                            {weekDay}
                        </div>
                    )
                })}
