...
export function NewHabitForm(){
    ...
    function handleToggleWeekDay(weekDay:number){
        if(weekDays.includes(weekDay)){
            const weekdaysUpdated = weekDays.filter(day=>day!=weekDay)
            setWeekDays(weekdaysUpdated)
        }else{
            const weekdaysUpdated = [...weekDays,weekDay]
            setWeekDays(weekdaysUpdated)
        }
    }
    ...
    return(
        <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
            ...
            className="p-4 rounded-lg mt-3 bg-zinc-800 text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-violet-600 focus:ring-offset-2 focus:ring-offset-zinc-900 transition-colors"
            autoFocus
            value={title}
            ...
            <div className="flex flex-col gap-2 mt-3 focus:outline-none" >
                {
                    avaliableWeekDays.map((weekDay,index)=>{
                        return(
                            <Checkbox.Root key={index} className='flex items-center gap-3 group focus:outline-none' checked={weekDays.includes(index)}
